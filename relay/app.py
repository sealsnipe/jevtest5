...
Inbound (default): long polling. The relay asks Telegram for updates itself
(getUpdates, like OpenClaw does), so no public URL, tunnel or DNS is needed for
receiving. Set RELAY_MODE=webhook to receive via POST /telegram instead.
The relay additionally runs Layer 1+3 (gatekeeper) so Grok Bot is only woken for
real requests, transcribes voice notes locally, and offers POST /reply for the
secretary bot (local TTS + Telegram sendVoice/sendMessage).
...
def process_update(update: dict[str, Any]) -> dict[str, Any]:
    """Handle one Telegram update (from webhook or poller). Returns a small status dict."""
...
async def telegram(
    ...
    """Webhook receiver (RELAY_MODE=webhook). Always answers 200, otherwise Telegram retries."""
...
# --- Inbound via long polling (default, like OpenClaw) ------------------------------

POLL_MODE = os.environ.get("RELAY_MODE", "poll") == "poll"
HEARTBEAT = LOG_DIR / "poll_heartbeat.txt"
OFFSET_FILE = LOG_DIR / "poll_offset.txt"
...
def _poll_loop() -> None:
    ...
    try:
        requests.post(f"{api}/deleteWebhook", data={"drop_pending_updates": "false"}, timeout=15)
    except requests.RequestException as e:
        _log("error", {"reason": f"deleteWebhook: {e}"})
    offset = 0
    try:
        offset = int(OFFSET_FILE.read_text().strip() or 0)
    except (OSError, ValueError):
        pass
    _log("info", {"event": "poll_start", "offset": offset})
    backoff = 2
    while True:
        try:
            r = requests.get(f"{api}/getUpdates", params={"offset": offset, "timeout": 25, "allowed_updates": '["message"]'}, timeout=35)
            data = r.json()
            if not data.get("ok"):
            ...
            for upd in data.get("result", []):
                ...
                try:
                    ...
                    _log("error", {"reason": f"process_update: {e}", "update_id": upd.get("update_id")})
            HEARTBEAT.write_text(datetime.now(timezone.utc).isoformat(timespec="seconds"))
            backoff = 2
        except Exception as e:  # noqa: BLE001
            _log("error", {"reason": f"poll: {e}"})
            time.sleep(backoff)
...
def _start_poller() -> None:
    if POLL_MODE and os.environ.get("TELEGRAM_BOT_TOKEN") and os.environ.get("RELAY_POLL", "1") != "0":
        LOG_DIR.mkdir(parents=True, exist_ok=True)
        threading.Thread(target=_poll_loop, name="telegram-poller", daemon=True).start()
